...
**Текущая версия:** см. `package.json` (`version`). До MVP используется **`0.ФАЗА.патч`**: второе число — **номер фазы** дорожной карты, третье — патчи внутри фазы. Текущая отметка **0.0.0** — завершена фаза 0 (инфраструктура); **1.0.0** — релиз MVP. Подробнее: `obsidian/10 — Версионирование релизов.md`, полный план фаз — `obsidian/11 — Дорожная карта до MVP (фазы).md`.
