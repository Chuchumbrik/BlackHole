...
function randomBoundaryVelocity(
  ...
  // Калибровка v0.5.73 (анализ doc 17): 1.6 → 1.1. Разброс ±~0.55 рад — компромисс
  // «честность ↔ воронка»: часть тел пролетает мимо (слабая ранняя гравитация
  // сохраняется), но захват достаточен, чтобы апгрейды зримо растили доход
  // (прежние 1.6 давали плато ~8 MP/с — большинство тел уходило мимо).
