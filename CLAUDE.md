...
`obsidian/` (RU). Канон: `01` (цели MVP), `02` (core loop), `07` (баланс/формулы), `11` (дорожная карта по фазам), `13` (баланс/темп). **`15` — Карта функциональностей (декомпозиция): живой справочник по всем частям игры (файлы, ключевые функции, статус). Обращаться к нему вместо повторной декомпозиции и ОБНОВЛЯТЬ при заметных изменениях логики.** Полная копия — в `/root/Projects/brain/projects/cosmo-blackhole/`. Каталог `obsidian/` исключён из деплоя (`.vercelignore`).
